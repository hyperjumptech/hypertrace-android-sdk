...
import tech.hyperjump.hypertrace.bluetooth.gatt.ACTION_DEVICE_PROCESSED
import tech.hyperjump.hypertrace.bluetooth.gatt.CONNECTION_DATA
import tech.hyperjump.hypertrace.bluetooth.gatt.DEVICE_ADDRESS
import tech.hyperjump.hypertrace.idmanager.TempIDManager
import tech.hyperjump.hypertrace.logging.CentralLog
import tech.hyperjump.hypertrace.protocol.BlueTrace
import tech.hyperjump.hypertrace.services.BluetoothMonitoringService
import tech.hyperjump.hypertrace.services.BluetoothMonitoringService.Companion.blacklistDuration
import tech.hyperjump.hypertrace.services.BluetoothMonitoringService.Companion.maxQueueTime
import tech.hyperjump.hypertrace.services.BluetoothMonitoringService.Companion.useBlacklist
import java.util.*
...
class StreetPassWorker(val context: Context) {
    ...
    private val serviceUUID: UUID = UUID.fromString(HyperTraceSdk.CONFIG.bleServiceUuid)
    private val characteristicV2: UUID = UUID.fromString(HyperTraceSdk.CONFIG.bleCharacteristicUuid)
...
//                || blacklist.filter { it.uniqueIdentifier == work.connectable.manuData }.isNotEmpty()
    ...
    fun finishWork(work: Work) {
        ...
        if (work.isCriticalsCompleted()) {
            BluetoothServiceUtil.broadcastDeviceProcessed(context, work.device.address)
                    ...
                    if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                        ...
                        try {
                            ...
                            connectionRecord?.let {
                                BluetoothServiceUtil.broadcastStreetPassReceived(
                                    context,
                                    connectionRecord
                                )
                            }
